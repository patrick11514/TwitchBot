...
class Logger {
    ...
    logToFile(type: 'INFO' | 'ERROR', data: any, ms?: number) {
        ...
        if (this.sensitive) {
            console.log(
                `${clc.white('[')}${clc.green(this.getTime())}${clc.white(']')} ${clc.white('[')}${textType}${clc.white(']')} ${clc.white('[')}${clc[this.color](this.name)}${clc.white(']')}`,
                data,
                ms ? `${clc.white(`(${ms} ms)`)}` : '',
            )
        } else {
            const formattedMessage = `${clc.white('[')}${clc.green(this.getTime())}${clc.white(']')} ${clc.white('[')}${textType}${clc.white(']')} ${clc.white('[')}${clc[this.color](this.name)}${clc.white(']')} ${textColor(data)} ${ms ? `${clc.white(`(${ms} ms)`)}` : ''}`
